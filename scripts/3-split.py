...
print("Writing out so much stuff")
write_all(df)
print("Apply different grouping function")
df["G1"] = df.FonetiskFornavn.apply(lambda x: x[1][:2].replace(" ", "_"))
df["G2"] = df.FonetiskFornavn.apply(lambda x: x[1][:4].replace(" ", "_"))
df["G3"] = df.FonetiskFødested.apply(lambda x: x[1][:2].replace(" ", "_"))
write_all(df)
